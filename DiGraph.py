...
class DiGraph(object):
    ...
    def Dijkstra(self, source):
        ...
        for v in self.vertices():           # Initializations
            dist[v] = self.getInfinity()    # Mark distances from source to v as not yet computed
        ...
        while q:                            # The main loop
            u = heapq.heappop(q)            # vertex in Q with smallest distance in dist 
            if visited[u]:                  # and has not been visited (Source node in first case)
                continue
            ...
            for e in self.adjacentTo(u):
                ...
                if alt < dist[v]:
                    ...
                    previous[v] = u
                    if not visited[v]:
                        heapq.heappush(q,v) # Add unvisited v into the Q to be processed
